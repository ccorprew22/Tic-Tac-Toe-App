...
export function Display() {
    ...
    if (data !== undefined) {
      if (data[0] === true && data[1] === 1) {
        ...
        for (let i = 0; i < data[3].length; i++) {
          if (data[2][0] === data[3][i].sid) {
            // get x player username index
            x_index = i;
          } else if (data[2][1] === data[3][i].sid) {
            // get y player username index
            y_index = i;
          }
  ...
  if (result === 'Player Disconnect! Game Over') {
    display = (
      <div className="">
        ...
        <Replay className="text-center" key={11} name="New Player Game" />
      </div>
    );
    ...
    display = (
      <div className="">
        <h1 className="text-center display">{result}</h1>
        <Replay className="text-center" key={11} name="Rematch" />
      </div>
    );
  } else if (result.length === 0) {
    // socket.emit("restart")
  ...
  return <div className="mx-auto">{display}</div>;
}
